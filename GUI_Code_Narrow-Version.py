...
def bonus_check(self):
    if self.balance >= 50.00:
        bonus_message.set("You're on track to recieve you're allocated bonus.\nMake sure to keep at least $50.00 in your account \ntill the end of the year to recieve the bonus! :)")
    else:
        bonus_message.set("You're not on track to receive you're allocated bonus.\nTry save up at least $50.00 in your account \ntill then end of the year to recieve the bonus! :)")
...
def deposit_money(account):
    ...
    if account.deposit(amount.get()):
        action_feedback.set("Your transaction has been completed. \nTotal of ${:.2f} has been deposited into {}".format(amount.get(), account.name))
    else:
...
def withdraw_money(account):
    ...
    if account.withdraw(amount.get()):
        action_feedback.set("Your transaction has been completed. \nTotal of ${:.2f} has been withdrawn from {}".format(amount.get(), account.name))
    else:
        action_feedback.set("Sorry, you either do not have enough money\n in {} or have written an invalid amount".format(account.name))
...
def manage_action():
    ...
    #Add an exception for text input
        ...
        action_feedback.set("Please enter a positive number (a number larger or equal to 0.01). \nSpecial characters and letters will not be accepted!")
...
top_frame = tkinter.LabelFrame(root, text="Account Details", font=("Garamond", 12, "bold"), bg='#000000', fg='#FFFFFF') #Garamond, Garamond, Garamond
...
message_label = tkinter.Label(top_frame, textvariable=message_text, wraplength=1000, font=("Garamond", 11), bg='#000000', fg='#FFFFFF')
...
details_label = tkinter.Label(top_frame, textvariable=account_details, font=("Garamond", 11, "bold"), bg='#2F8EA1', fg='#FFFFFF')
...
account_label = tkinter.Label(bottom_frame, text="Account: ", font=("Garamond", 11, "bold"), bg='#000000', fg='#FFFFFF')
...
account_box = ttk.Combobox(bottom_frame, textvariable=chosen_account, state="readonly", font=("Garamond", 9))
...
action_label = tkinter.Label(bottom_frame, text="Action:", font=("Garamond", 11, "bold"), bg='#000000', fg='#FFFFFF')
...
action_box = ttk.Combobox(bottom_frame, textvariable=chosen_action, state="readonly", font=("Garamond", 10))
action_box['values'] = action_list
action_box.grid(row=4, column=1, padx=10, pady=3, sticky="WE")

#Create a label for the amount field and pack it into the GUI
amount_label = tkinter.Label(bottom_frame, text="Amount:", font=("Garamond", 11, "bold"), bg='#000000', fg='#FFFFFF')
...
amount_entry = tkinter.Entry(bottom_frame, textvariable=amount, font=("Garamond", 10), bg='#FFFFFF', fg='#000000')
amount_entry.grid(row=5, column=1, padx=10, pady=3, sticky="WE")

#Create a submit button
submit_button = tkinter.Button(bottom_frame, text="Submit", command=manage_action, font=("Garamond", 11, "bold"), bg='#2F8EA1', fg='#FFFFFF')
...
action_feedback_label = tkinter.Label(bottom_frame, textvariable=action_feedback, font=("Garamond", 12), bg='#000000', fg='#FFFFFF')
...
bonus_label = tkinter.Label(bottom_frame, textvariable=bonus_message, wraplength=1000, font=("Garamond", 12), bg='#000000', fg='#FFFFFF')
